...
root.title("Christine Njeri-JKUATES LTD")
toplbl=Label(root,text="HOTEL MANAGEMENT SYSTEM",font=('arial',36,'bold'))
toplbl.place(x=200,y=0)
mycanvas=Canvas(root,width=1300,height=5)
line=mycanvas.create_line(0,0,1300,0,width=10,fill="deep sky blue")
...
line2=mycanvas2.create_line(0,0,620,0,width=15,fill="midnight blue")
...
load.title("loading...")
load.configure(bg="dark magenta")
pl=Label(load,font=("arial 18 bold"),fg="cyan",bg="dark magenta")
pl.place(x=170,y=60)
pb=Progressbar(load,orient='horizontal',mode='determinate',length=300,)
pb.place(x=20,y=120)
label=Label(load,text="Login Form Loading",fg='black',font=("arial 18 bold"),bg="dark magenta")
label.pack(side=TOP)
label2=Label(load,text="Please Wait....",bg="dark magenta")
...
def login():
    if entry1.get() == '' and entry2.get() == '':
        success.set("Enter your login details!")
    else:
        if entry1.get() == 'muhoro' and entry2.get() == 'njeri':
            root.deiconify()
            top.destroy()
        else:
            success.set("Wrong username or password!!")
global success
success=StringVar()
...
top.title("Christine Njeri.LTD-JKUATES LIMITED-year 2022-June")
top.configure(bg='medium slate blue')

lbl7=Label(top,text='USER LOGIN',font=('times',15,'bold'),fg='black',bg='medium slate blue',bd=15)
lbl0=Label(top,text='The Muhoros\' Hotel Management System',font=('times',15),
           bg='medium slate blue',bd=15)
lbl1=Label(top,text='Username',font=('times',10,'bold'),bg='medium slate blue',bd=15)
lbl2=Label(top,text='Password',font=('times',10,'bold'),bg='medium slate blue',bd=15)
entry1=Entry(top)
entry2=Entry(top,show="*")
lbl=Label(top,text="",textvariable=success,font=('arial',12),bg='medium slate blue',fg="maroon")
btn2=Button(top,text='Cancel',fg='black',bg='dark slate grey',font=('arial',15,'bold'),command=exit1)
btn1=Button(top,text="Login",fg='black',bg='dark slate grey',font=('arial',15,'bold'),command=login)
lbl8=Label(top,text='By: Christine Njeri Muhoro ',font=('Helvetica',8),bg='medium slate blue')
lbl0.place(x=20,y=0)
...
def Total():
    ...
    sentense = ("    THE MUHOROS\' HOTEL\n" + "Hello, " + lblentry.get() + "\n Items   Amount   price"
        ...
        sodaentry.get()) + "      " +
                ...
                str(totalentry.get()) +
                "\n\nThank you for visiting The Muhoros\' Hotel ")
